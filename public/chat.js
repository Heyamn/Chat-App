...
function countMessages() {
    if(messagesSent >= 4) {
        if(localStorage.getItem('timesSpamming') == 0) {
            ...
            createMessage('./profile-imgs/mod-bot.png', 'Moderator', 'I detected that you were spamming... First warning, on the third you are blocked', true)

        }else if(localStorage.getItem('timesSpamming') == 1) {

            localStorage.setItem('timesSpamming', 2)
            createMessage('./profile-imgs/mod-bot.png', 'Moderator', 'I detected that you were spamming... Last warning, once again you spam you are blocked', true)

        }else if(localStorage.getItem('timesSpamming') == 2) {

            localStorage.setItem('timesSpamming', 3)

            createMessage('./profile-imgs/mod-bot.png', 'Moderator', 'Unfortunately you were permanently banned for spamming', true)

            socket.emit('new chat message', `${userName} permanently banned for spamming`, 'Moderator', './profile-imgs/mod-bot.png', true)
...
sendMessageForm.addEventListener('submit', e => {
    ...
    if (!messageInput.value.trim() == '' && localStorage.getItem('userBlocked') == 'false') {
        ...
        createMessage('./profile-imgs/mod-bot.png', 'moderator', 'You are permanently in "Read Only" mode for spamming', true)
...
newNameForm.addEventListener('submit', e => {
    ...
    if (!newNameInput.value.trim() == '') {
        ...
        // Socket emit a new message
        socket.emit('new chat message', `${lastUserName} now it's called ${userName}`, 'Bot', './profile-imgs/names-bot.png', true)

        // Create message
        createMessage('./profile-imgs/names-bot.png', 'Bot', ` ${lastUserName} now it's called  ${userName}`, true)
...
socket.on('i connected', personConnectedName => {
    // Create bot message
    createMessage('./profile-imgs/bot.png', 'Connections bot', `${ personConnectedName }  is online`, true)
...
socket.on('someone disconnected', personDisconnectedName => {
    // Create bot message
    createMessage('./profile-imgs/bot.png', 'Connections bot', `${ personDisconnectedName } went offline`, true)
